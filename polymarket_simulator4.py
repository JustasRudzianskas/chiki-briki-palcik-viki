...
if "initialized" not in st.session_state:
    # First ever startup — mark current unix timestamp as our start point.
    # We'll only track activity that happens AFTER this moment.
    # Also bulk-insert all existing transaction hashes into seen_trades
    # so historical trades are permanently skipped even after restarts.
    now_ts = int(datetime.now(timezone.utc).timestamp())
    st.session_state.tracking_since = now_ts

    # Fetch all historical activity and mark every tx as seen without executing
    try:
        offset = 0
        limit = 100
        while True:
    ...
    # App restarted mid-session — restore last_checked from DB-persisted seen_trades.
    # We don't know the exact timestamp but we know which trades we've seen,
    # so just set last_checked to now and rely on seen_trades to deduplicate.
...
def fetch_new_activity(wallet, since_timestamp):
    """
    Fetch only activity that occurred after since_timestamp (unix seconds).
    Uses ASC order so BUYs always come before their REDEEMs in processing.
    Paginates to catch every trade — even bursts of many trades in seconds.
    """
    all_activity = []
    limit = 100
    offset = 0
    ...
    if activity_type == "TRADE":
        ...
        if side == "BUY":
            ...
            if token_id in st.session_state.positions:
                ...
                # Use actual usdcSize received as proceeds for accuracy
                try:
                    proceeds = float(activity.get("usdcSize", price * pos["size"]))
                except (ValueError, TypeError):
        ...
        if token_id in st.session_state.positions:
            ...
            # Position not in state (e.g. bot started after the BUY)
            try:
            ...
            if usdc > 0:
                ...
                save_trade(market, token_id, "REDEEM", 1.0, size, 0, "REDEEMED")
...
# Advance the window — next refresh only fetches activity after now
st.session_state.last_checked = int(datetime.now(timezone.utc).timestamp())
