...
def execute_paper_trade(activity):
    ...
    if activity_type == "TRADE":
        ...
        if side == "BUY":
            ...
            # Use session state, fall back to DB reconstruction
            if token_id in st.session_state.positions:
                pos = st.session_state.positions[token_id]
            else:
...
if new_activity:
    ...
    # Subtract 1 so we don't miss trades with the exact same timestamp
    st.session_state.last_checked = newest_ts - 1
else:
