...
def fetch_wallet_activity(wallet):
    """
    Fetch all wallet activity from Polymarket with pagination.
    Fetches TRADE and REDEEM types — these are the only ones we act on.
    """
    all_activity = []
    limit = 100
    offset = 0
    ...
    while True:
        url = (
            ...
            f"?user={wallet}&limit={limit}&offset={offset}&type=TRADE&type=REDEEM"
        )
        try:
    ...
    if activity_type == "TRADE":
        ...
        if side == "BUY":
            ...
            if token_id in st.session_state.positions:
                ...
                # Return sale proceeds only — pnl is already included
        ...
        # A redeem means the market resolved and the user held winning tokens.
        # Winning tokens always pay out at $1.00 each.
        # usdcSize is the actual cash received — use it directly if available.
        # In paper trading: close the position at price=1.0 and credit the payout.

        if token_id in st.session_state.positions:
            pos = st.session_state.positions[token_id]
            redeem_price = 1.0  # winning tokens always resolve at $1
            ...
            # Position not tracked (e.g. bot started after the BUY was made).
            # Still log it using usdcSize so it shows in history.
            try:
